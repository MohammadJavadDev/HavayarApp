...
public class UserService (IAuthService _authService, ApplicationDbContext db, ISdk? sdk) : IUserService
{
    ...
    public async Task<AuthenticateResponse?> Authenticate(AuthenticateRequest model)
    {
	 ...
	 if (user == null || !PasswordHasher.VerifyPassword(user.Password , model.Password)) return null;
    ...
    {
        var user = ListUsers.SystemUsers.FirstOrDefault(c => c.Id == userId);

        if (user == null)
        {
            ListUsers.SystemUsers = db.Users.Select(c => new User() { Id = c.Id, Name = c.Name }).ToList();
            user = ListUsers.SystemUsers.FirstOrDefault(c => c.Id == userId);
        }
        return user;
    }
