...
namespace Data.Services.QueryBuilderServices
{
		...
		private static readonly HashSet<string> DangerousPatterns = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
    {
	   ...
	   "openrowset", "opendatasource", "INFORMATION_SCHEMA"
		...
		{
			// تنها SELECT، WITH (CTE) و EXEC (Stored Procedure) مجاز است
			var trimmedQuery = query.Trim().ToUpper();

			if (!trimmedQuery.StartsWith("SELECT") && !trimmedQuery.StartsWith("WITH")
			    && !trimmedQuery.StartsWith("EXEC ") && !trimmedQuery.StartsWith("EXECUTE "))
			{
				result.IsValid = false;
				result.Errors.Add("تنها دستورات SELECT، CTE (WITH) و EXEC (Stored Procedure) مجاز هستند");
			}
