/*
Seed_CngInvoice_RolesAndAccess.sql
دسترسی فاکتور CNG مطابق HTS صفحه 476:
  403 کارشناسان فاکتور CNG → Cng.Invoice.Manage (CRUD + excel؛ بدون ShowAll فیلتر)
  404 مدیران فاکتور CNG → Cng.Invoice.Full (CRUD + excel + ShowAll)
  350 مشاهده کلی HTS → نقش موجود مشاهده کلی HTS — list/fetch/edit/excel فقط؛ بدون ShowAll
  ShowAllMenus → همه مسیرها
  بدون اکشن صفحه‌ای مستقیم کاربر روی صفحه 476
...
BEGIN TRY
    ...
    DECLARE @SeedUser NVARCHAR(150) = N'seed-cng-invoice-roles';
    ...
    INSERT INTO #CngRoles (WantId, Name, Title) VALUES
        (600019, N'Cng.Invoice.Manage', N'کارشناسان فاکتور CNG (HTS 403)'),
        (600020, N'Cng.Invoice.Full', N'مدیران فاکتور CNG (HTS 404)');
    ...
    -- View: list/fetch/edit + excel for Manage + Full + ShowAllMenus + مشاهده کلی
    INSERT INTO #Map SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/cng/invoice/list', 1, 1),
        (N'/panel/cng/invoice/fetchdata', 2, 2),
        (N'/panel/cng/invoice/edit', 1, 5),
        (N'/panel/cng/invoice/exporttoexcel', 2, 0)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES
    ...
    -- Manage + Full + ShowAllMenus: CRUD
    INSERT INTO #Map SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/cng/invoice/new', 1, 4),
        (N'/panel/cng/invoice/save', 2, 3),
        (N'/panel/cng/invoice/add', 2, 4),
        (N'/panel/cng/invoice/update', 2, 5),
        (N'/panel/cng/invoice/delete', 2, 6)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES
        (N'ShowAllMenus'), (N'Cng.Invoice.Manage'), (N'Cng.Invoice.Full')
    ) r(RoleName);

    DELETE ra FROM system.RoleAccess ra
    INNER JOIN system.Role r ON r.Id = ra.RoleId
    WHERE ra.Path LIKE N'/panel/cng/invoice/%'
      AND r.Name IN (
            N'Cng.Invoice.Manage', N'Cng.Invoice.Full',
            N'ShowAllMenus', N'مشاهده کلی HTS'
      );
    ...
    PRINT N'=== [4] Assign HTS groups 403/404 users (if TMS linked) ===';
    ...
    BEGIN
        ...
        -- 403 Manage (by UserGroup membership; page 476 has no direct user page-actions)
        INSERT INTO #UserRoleMap (Username, RoleName)
        SELECT DISTINCT COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')), N'Cng.Invoice.Manage'
        FROM [TMS].[TotalSystem].[dbo].[Gnr_UserGroupMember] m
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_User] u ON u.User_ID = m.User_FK
        WHERE m.UserGroup_FK = 403
          AND COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) IS NOT NULL;

        -- 404 Full
        INSERT INTO #UserRoleMap (Username, RoleName)
        SELECT DISTINCT COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')), N'Cng.Invoice.Full'
        FROM [TMS].[TotalSystem].[dbo].[Gnr_UserGroupMember] m
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_User] u ON u.User_ID = m.User_FK
        WHERE m.UserGroup_FK = 404
          AND COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) IS NOT NULL
          AND NOT EXISTS (
                ...
                WHERE x.Username = COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N''))
                  AND x.RoleName = N'Cng.Invoice.Full'
          );
    ...
    COMMIT TRANSACTION;
    PRINT N'=== DONE Seed_CngInvoice_RolesAndAccess ===';
END TRY
BEGIN CATCH
...
SELECT Id, Name, Title FROM system.Role WHERE Name LIKE N'Cng.Invoice%' ORDER BY Id;
