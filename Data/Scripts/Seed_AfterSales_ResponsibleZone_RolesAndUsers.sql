/*
  Seed_AfterSales_ResponsibleZone_RolesAndUsers.sql
  مسئولین مناطق مطابق HTS صفحه 356 Sale_ResponsibleZone:
    گروه 207 New/Edit/Delete → Manage
    گروه 350 Read → View
    اعطای مستقیم روی 356 وجود ندارد.
  اکسل در HTS روی گرید سربرگ نیست؛ در Havayar فقط به Manage / AfterSales / ShowAllMenus داده شد.
...
BEGIN TRY
    ...
    DECLARE @SeedUser NVARCHAR(150) = N'seed-aftersales-responsiblezone-roles';
    DECLARE @EntityPascal NVARCHAR(200) = N'Entities.App.Sale.ResponsibleZone';
    DECLARE @ChildPascal NVARCHAR(200) = N'Entities.App.Sale.ResponsibleZoneCustomer';

    PRINT N'=== [1] Roles 500026 / 500027 ===';
    IF OBJECT_ID('tempdb..#RzRoles') IS NOT NULL DROP TABLE #RzRoles;
    CREATE TABLE #RzRoles (WantId BIGINT NOT NULL, Name NVARCHAR(200) NOT NULL, Title NVARCHAR(200) NOT NULL);
    INSERT INTO #RzRoles (WantId, Name, Title) VALUES
    ...
    DECLARE rc CURSOR LOCAL FAST_FORWARD FOR SELECT WantId, Name, Title FROM #RzRoles;
    ...
    IF OBJECT_ID('tempdb..#RzActionMap') IS NOT NULL DROP TABLE #RzActionMap;
    CREATE TABLE #RzActionMap
    (
    ...
    INSERT INTO #RzActionMap (RoleName, Path, ActionAccessType, ActionAccessItemType, EntityName)
    SELECT r.RoleName, a.Path, a.AType, a.IType, a.Ent
    FROM (VALUES
        (N'/panel/sale/responsiblezone/list', 1, 1, N'Entities.App.Sale.ResponsibleZone'),
        (N'/panel/sale/responsiblezone/fetchdata', 2, 2, N'Entities.App.Sale.ResponsibleZone'),
        (N'/panel/sale/responsiblezonecustomer/list', 1, 1, N'Entities.App.Sale.ResponsibleZoneCustomer'),
        (N'/panel/sale/responsiblezonecustomer/fetchdata', 2, 2, N'Entities.App.Sale.ResponsibleZoneCustomer'),
        (N'/panel/sale/responsiblezonecustomer/listbyparentid', 1, 1, N'Entities.App.Sale.ResponsibleZoneCustomer'),
        (N'/panel/sale/responsiblezonecustomer/getlistbyparentid', 2, 2, N'Entities.App.Sale.ResponsibleZoneCustomer')
    ) a(Path, AType, IType, Ent)
    CROSS JOIN (VALUES
    ...
    SELECT r.RoleName, a.Path, a.AType, a.IType, a.Ent
    FROM (VALUES
        (N'/panel/sale/responsiblezone/save', 2, 3, N'Entities.App.Sale.ResponsibleZone'),
        (N'/panel/sale/responsiblezone/add', 2, 4, N'Entities.App.Sale.ResponsibleZone'),
        (N'/panel/sale/responsiblezone/update', 2, 5, N'Entities.App.Sale.ResponsibleZone'),
        (N'/panel/sale/responsiblezone/delete', 2, 6, N'Entities.App.Sale.ResponsibleZone'),
        (N'/panel/sale/responsiblezone/edit', 1, 5, N'Entities.App.Sale.ResponsibleZone'),
        (N'/panel/sale/responsiblezone/new', 1, 4, N'Entities.App.Sale.ResponsibleZone'),
        (N'/panel/sale/responsiblezone/exporttoexcel', 2, 0, N'Entities.App.Sale.ResponsibleZone'),
        (N'/panel/sale/responsiblezonecustomer/save', 2, 3, N'Entities.App.Sale.ResponsibleZoneCustomer'),
        (N'/panel/sale/responsiblezonecustomer/add', 2, 4, N'Entities.App.Sale.ResponsibleZoneCustomer'),
        (N'/panel/sale/responsiblezonecustomer/update', 2, 5, N'Entities.App.Sale.ResponsibleZoneCustomer'),
        (N'/panel/sale/responsiblezonecustomer/delete', 2, 6, N'Entities.App.Sale.ResponsibleZoneCustomer'),
        (N'/panel/sale/responsiblezonecustomer/edit', 1, 5, N'Entities.App.Sale.ResponsibleZoneCustomer'),
        (N'/panel/sale/responsiblezonecustomer/new', 1, 4, N'Entities.App.Sale.ResponsibleZoneCustomer'),
        (N'/panel/sale/responsiblezonecustomer/exporttoexcel', 2, 0, N'Entities.App.Sale.ResponsibleZoneCustomer')
    ) a(Path, AType, IType, Ent)
    CROSS JOIN (VALUES
        (N'ShowAllMenus'), (N'Sale.AfterSales'),
        (N'Sale.AfterSales.ResponsibleZone.Manage')
    ...
    WHERE (ra.Path LIKE N'/panel/sale/responsiblezone/%' OR ra.Path LIKE N'/panel/sale/responsiblezonecustomer/%')
      AND r.Name IN (
            N'Sale.AfterSales.ResponsibleZone.Manage',
            N'Sale.AfterSales.ResponsibleZone.View',
    ...
    PRINT N'=== [4] Assign HTS page 356 users ===';
    ...
    FROM (
        SELECT COALESCE(g.AdName, g.HtsUsername) AS Username,
               N'Sale.AfterSales.ResponsibleZone.Manage' AS RoleName, g.Source
        FROM #HtsGrant g
        WHERE g.Permission_ID IN (4, 5, 6) AND COALESCE(g.AdName, g.HtsUsername) IS NOT NULL

        UNION ALL
        SELECT COALESCE(g.AdName, g.HtsUsername),
               N'Sale.AfterSales.ResponsibleZone.View', g.Source
        FROM #HtsGrant g
    ...
    DECLARE @ViewRoleId bigint = (SELECT Id FROM system.Role WHERE Name = N'Sale.AfterSales.ResponsibleZone.View');
    DECLARE @ManageRoleId bigint = (SELECT Id FROM system.Role WHERE Name = N'Sale.AfterSales.ResponsibleZone.Manage');
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT @StripRoles = N'[' + STUFF((
                ...
                WHERE j.value <> N'Sale.AfterSales.ResponsibleZone.View'
                FOR XML PATH(''), TYPE).value('.', 'nvarchar(max)'), 1, 1, N'') + N']';
    ...
    COMMIT TRANSACTION;
    PRINT N'=== DONE Seed_AfterSales_ResponsibleZone_RolesAndUsers ===';
END TRY
BEGIN CATCH
...
SELECT r.Id, r.Name, r.Title,
       (SELECT COUNT(*) FROM system.RoleAccess ra
        WHERE ra.RoleId = r.Id AND (ra.Path LIKE N'/panel/sale/responsiblezone%' OR ra.Path LIKE N'dataProfile_%')) AS RzAccess,
