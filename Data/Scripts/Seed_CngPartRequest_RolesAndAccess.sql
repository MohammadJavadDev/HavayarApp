/*
Seed_CngPartRequest_RolesAndAccess.sql
دسترسی درخواست کالا CNG مطابق HTS صفحه 453:
  364 کارشناسان درخواست کالا (CNG) → Cng.PartRequest.Manage (CRUD + sendprefactor/download/getitems؛ بدون Done و بدون ShowAll فیلتر)
  365 کارشناسان درخواست کالا (CNG) - تغییر وضعیت → Cng.PartRequest.Status (همان + Done + ShowAll)
  350 مشاهده کلی HTS → نقش موجود مشاهده کلی HTS — list/fetch/edit فقط
  کاربر مستقیم ShowAll (Permission 7، خارج از 365، مثلاً Piri.m) → Cng.PartRequest.ShowAll — list/fetch/edit
  ShowAllMenus → همه مسیرها از جمله Done
  بدون exporttoexcel؛ GetStock در RoleAccess نیست
...
BEGIN TRY
    ...
    DECLARE @SeedUser NVARCHAR(150) = N'seed-cng-partrequest-roles';
    ...
    INSERT INTO #CngRoles (WantId, Name, Title) VALUES
        (600013, N'Cng.PartRequest.Manage', N'کارشناسان درخواست کالا (CNG) (HTS 364)'),
        (600014, N'Cng.PartRequest.Status', N'کارشناسان درخواست کالا (CNG) - تغییر وضعیت (HTS 365)'),
        (600015, N'Cng.PartRequest.ShowAll', N'نمایش همه درخواست کالا CNG');
    ...
    -- View: list/fetch/edit + getitems/download for Manage + Status + ShowAll + ShowAllMenus + مشاهده کلی
    INSERT INTO #Map SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/cng/partrequest/list', 1, 1),
        (N'/panel/cng/partrequest/fetchdata', 2, 2),
        (N'/panel/cng/partrequest/edit', 1, 5),
        (N'/panel/cng/partrequest/getitems', 2, 2),
        (N'/panel/cng/partrequest/download', 2, 1000)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES
    ...
    -- Manage + Status + ShowAllMenus: CRUD + sendprefactor (NO exporttoexcel, NO GetStock)
    INSERT INTO #Map SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/cng/partrequest/new', 1, 4),
        (N'/panel/cng/partrequest/save', 2, 3),
        (N'/panel/cng/partrequest/add', 2, 4),
        (N'/panel/cng/partrequest/update', 2, 5),
        (N'/panel/cng/partrequest/delete', 2, 6),
        (N'/panel/cng/partrequest/sendprefactor', 2, 1000)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES
        (N'ShowAllMenus'), (N'Cng.PartRequest.Manage'), (N'Cng.PartRequest.Status')
    ) r(RoleName);

    -- Done: Status + ShowAllMenus only
    INSERT INTO #Map SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/cng/partrequest/done', 2, 1000)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES (N'ShowAllMenus'), (N'Cng.PartRequest.Status')) r(RoleName);

    DELETE ra FROM system.RoleAccess ra
    INNER JOIN system.Role r ON r.Id = ra.RoleId
    WHERE ra.Path LIKE N'/panel/cng/partrequest/%'
      AND r.Name IN (
            N'Cng.PartRequest.Manage', N'Cng.PartRequest.Status', N'Cng.PartRequest.ShowAll',
            N'ShowAllMenus', N'مشاهده کلی HTS'
      );
    ...
    SELECT DISTINCT m.Path, m.AType, m.IType,
        N'Entities.App.Cng.PartRequest',
        NULL, NULL, NULL, r.Id,
    ...
    PRINT N'=== [4] Assign HTS page 453 users (if TMS linked) ===';
    ...
    BEGIN
        ...
        SELECT DISTINCT COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')), N'Cng.PartRequest.Manage'
        ...
        WHERE pug.UserGroup_FK = 364 AND pa.Page_FK = 453
          AND COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) IS NOT NULL;

        -- 365 Status
        INSERT INTO #UserRoleMap (Username, RoleName)
        SELECT DISTINCT COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')), N'Cng.PartRequest.Status'
        FROM [TMS].[TotalSystem].[dbo].[Gnr_PageAction_UserGroup] pug
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_PageAction] pa ON pa.PageAction_ID = pug.PageAction_FK
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_UserGroupMember] m ON m.UserGroup_FK = pug.UserGroup_FK
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_User] u ON u.User_ID = m.User_FK
        WHERE pug.UserGroup_FK = 365 AND pa.Page_FK = 453
          AND COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) IS NOT NULL
          AND NOT EXISTS (SELECT 1 FROM #UserRoleMap x WHERE x.Username = COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) AND x.RoleName = N'Cng.PartRequest.Status');

        -- 350 → مشاهده کلی HTS only (no Cng.PartRequest.View)
        INSERT INTO #UserRoleMap (Username, RoleName)
        ...
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_UserGroupMember] m ON m.UserGroup_FK = pug.UserGroup_FK
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_User] u ON u.User_ID = m.User_FK
        WHERE pug.UserGroup_FK = 350 AND pa.Page_FK = 453
          AND COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) IS NOT NULL
        ...
        -- Direct ShowAll (PageAction Permission_FK=7), exclude users already in group 365
        INSERT INTO #UserRoleMap (Username, RoleName)
        SELECT DISTINCT COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')), N'Cng.PartRequest.ShowAll'
        ...
        WHERE pa.Page_FK = 453 AND pa.Permission_FK = 7
          AND COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) IS NOT NULL
          AND NOT EXISTS (
                SELECT 1
                FROM [TMS].[TotalSystem].[dbo].[Gnr_PageAction_UserGroup] pug
                ...
                INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_UserGroupMember] m ON m.UserGroup_FK = pug.UserGroup_FK
                WHERE pug.UserGroup_FK = 365 AND pa2.Page_FK = 453 AND m.User_FK = u.User_ID
          )
          AND NOT EXISTS (
                ...
                WHERE x.Username = COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N''))
                  AND x.RoleName = N'Cng.PartRequest.ShowAll'
          );
    ...
    COMMIT TRANSACTION;
    PRINT N'=== DONE Seed_CngPartRequest_RolesAndAccess ===';
END TRY
BEGIN CATCH
...
SELECT Id, Name, Title FROM system.Role WHERE Name LIKE N'Cng.PartRequest%' ORDER BY Id;
