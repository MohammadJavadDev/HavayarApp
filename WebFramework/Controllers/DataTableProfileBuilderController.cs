...
namespace WebFramework.Controllers
{
	...
	public class DataTableProfileBuilderController(IUnitOfWork unitOfWork,
	  ...
	  IEntityMetadataCache entityMetadataCache) : BaseController
	{
		...
		public IActionResult GetEntityProp(string name)
		{
			...
			if (entityType != null)
			{
				...
				foreach (var prop in properties)
				{
					...
					string typeProperty = prop.DataType ?? "string";
					...
					else if (prop.SystemType == SystemType.Select)
					{
						//var enumType = prop.PropertyType;
						tmpColumnTable.options = prop.Options.Select(c => new DataTableOptions()
						{
							name = c.Text,
		    ...
		    string baseTableSchema)
		{
			...
			if (prop.SystemType == SystemType.Select)
			{
				select.options = prop.Options.Select(o => new OptionViewModel()
				{
					Name = o.Text,
