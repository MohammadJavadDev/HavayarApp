...
namespace WebApp.Controllers.Dynamic
{
	...
	[ApiController]
	[ApiResultFilter]
	[ControllerInfo("قیمت مصوب کالا", typeof(PartPrice))]
	public class PartPriceController(IUnitOfWork unitOfWork, IWebHostEnvironment env) : BaseController
	{
		[HttpPost("[action]")]
		...
		public async Task<IActionResult> Add(PartPrice model, CancellationToken cn)
			=> Ok(await unitOfWork.Repository<PartPrice>().SaveAsync(model, cn, true));

		[HttpPost("[action]")]
		[ActionDisplayName("ویرایش", ActionAccessType.Api, ActionAccessItemType.Update)]
		public async Task<IActionResult> Update(PartPrice model, CancellationToken cn)
			=> Ok(await unitOfWork.Repository<PartPrice>().UpdateAsync(model, cn, true));
		...
		public IActionResult Edit(long? id)
		{
			var entity = (id != null && id != 0)
				? unitOfWork.Repository<PartPrice>().TableNoTracking.FirstOrDefault(c => c.Id == id)
				: new PartPrice();
			return View(@"\Views\Panel\Sale\PartPrice\Edit.cshtml", entity ?? new PartPrice());
		}

		[HttpGet("[action]")]
		[ActionDisplayName("درج اطلاعات", ActionAccessType.View, ActionAccessItemType.Create)]
		public IActionResult New() => View(@"\Views\Panel\Sale\PartPrice\Edit.cshtml", new PartPrice());

		[HttpGet("[action]")]
		[ActionDisplayName("لیست اطلاعات", ActionAccessType.View, ActionAccessItemType.List)]
		public IActionResult List() => View(@"\Views\Panel\Sale\PartPrice\List.cshtml");
		...
		[HttpGet("[action]")]
		[ActionDisplayName("نمایش قیمت", ActionAccessType.View, ActionAccessItemType.Custom)]
