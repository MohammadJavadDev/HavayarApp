/*
================================================================================
  Provisioning: system.SavedQuery + dbo.ReportBuilderReport برای داشبورد مهندسی MDR
  مرجع زنده: SavedQuery.Id=124 ، ReportBuilderReport.Id=15 (قبلاً ساخته
  شده‌اند). این نسخه برای مستندسازی/تکرارپذیری نگه داشته
  می‌شود و متن @CustomQuery دقیقاً برابر است با محتوای
  Docs/Sql/Edms_MdrDashboard_Query.sql (بخش بعد از GO).

  برای یک محیط تازه، بخش UPDATE انتهای اسکریپت را با
  INSERT INTO system.SavedQuery(...)  +  INSERT INTO dbo.ReportBuilderReport(...)
  جایگزین کنید (مطابق الگوی تاریخچه این فایل در git).
================================================================================
*/

SET NOCOUNT ON;

DECLARE @CustomQuery NVARCHAR(MAX) = N'SET NOCOUNT ON;
...
DECLARE @Selects NVARCHAR(MAX) = N'[{"Index":0,"Name":"MdrDocumentHeaders","Title":"سرجمع مدارک MDR"},{"Index":1,"Name":"MdrDocumentIssuances","Title":"ارسال‌های مدارک"},{"Index":2,"Name":"MdrTransmittals","Title":"ترانسمیتال‌ها"}]';

DECLARE @QueryJson NVARCHAR(MAX) =
    N'{"Tables":[],"Relations":[],"Filters":[],"CustomConditions":null,"CustomQuery":"'
    + STRING_ESCAPE(@CustomQuery, 'json')
    + N'","Parameters":[],"Selects":' + @Selects + N'}';

UPDATE system.SavedQuery
SET QueryJson = @QueryJson,
    ModifiedDateMiladiDateTime = SYSUTCDATETIME(),
    ModifiedByName = N'db-provision-script'
WHERE Id = 124;

SELECT Id, Name, Title, ISJSON(QueryJson) AS QueryJsonValid, LEN(QueryJson) AS QueryJsonLen
FROM system.SavedQuery WHERE Id = 124;