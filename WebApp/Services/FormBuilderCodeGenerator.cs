...
namespace WebApp.Services
{
	...
	public class FormBuilderCodeGenerator : IFormBuilderCodeGenerator
	{
		...
		public FormBuilderCodeGenerator(IWebHostEnvironment webHostEnvironment)
		{
	...
	public Dictionary<string, string> GetFilePaths(FormDefinition formDefinition)
	{
		...
		private string GeneratePropertyCode(FormProperty prop, string parentEntityName)
		{
			...
			if (prop.SystemType == SystemType.ListEntity || prop.SystemType == SystemType.ListString || prop.SystemType == SystemType.ListLong)
			{
				nullable = "";
			}

			sb.AppendLine($"\t\tpublic {propertyType}{nullable} {prop.PropertyName} {{ get; set; }}{GetDefaultValue(prop)}");
		...
		private string GetDefaultValue(FormProperty prop)
		{
			...
			if (prop.SystemType == SystemType.Boolean && !prop.Required)
				return " = false";
