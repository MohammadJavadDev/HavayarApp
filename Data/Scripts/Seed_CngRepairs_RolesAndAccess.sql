/*
Seed_CngRepairs_RolesAndAccess.sql
دسترسی تعمیرات CNG مطابق HTS صفحه 383:
  231 کارشناسان تعمیرات CNG → Cng.Repairs.Manage (view/new/edit/delete/excel/fetch + children; NO confirm)
  232 کارشناسان تاییدکننده تعمیرات CNG → Cng.Repairs.Confirm (new/edit/delete/confirm + list/fetch; NO excel)
  350 مشاهده کلی HTS → Cng.Repairs.View (+ نقش موجود مشاهده کلی HTS) — list/fetch only; no ShowAll filter
...
BEGIN TRY
    ...
    DECLARE @SeedUser NVARCHAR(150) = N'seed-cng-repairs-roles';
    ...
    INSERT INTO #CngRoles (WantId, Name, Title) VALUES
        (600010, N'Cng.Repairs.Manage', N'کارشناسان تعمیرات CNG (HTS 231)'),
        (600011, N'Cng.Repairs.Confirm', N'کارشناسان تاییدکننده تعمیرات CNG (HTS 232)'),
        (600012, N'Cng.Repairs.View', N'مشاهده تعمیرات CNG (HTS 350)');
    ...
    -- View: list/fetch/edit(view) for Manage + Confirm + View + ShowAll + مشاهده کلی
    INSERT INTO #Map SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/cng/repairs/list', 1, 1),
        (N'/panel/cng/repairs/fetchdata', 2, 2),
        (N'/panel/cng/repairs/edit', 1, 5)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES
        (N'ShowAllMenus'), (N'مشاهده کلی HTS'),
        (N'Cng.Repairs.Manage'), (N'Cng.Repairs.Confirm'), (N'Cng.Repairs.View')
    ) r(RoleName);

    -- Manage: CRUD + excel
    INSERT INTO #Map SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/cng/repairs/new', 1, 4),
        (N'/panel/cng/repairs/save', 2, 3),
        (N'/panel/cng/repairs/add', 2, 4),
        (N'/panel/cng/repairs/update', 2, 5),
        (N'/panel/cng/repairs/delete', 2, 6),
        (N'/panel/cng/repairs/exporttoexcel', 2, 0),
        (N'/panel/cng/repairs/sendaspartentrynotify', 2, 1000)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES (N'ShowAllMenus'), (N'Cng.Repairs.Manage')) r(RoleName);

    -- Confirm role: new/edit/delete/confirm + send notify (no excel) — HTS 232
    INSERT INTO #Map SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/cng/repairs/new', 1, 4),
        (N'/panel/cng/repairs/save', 2, 3),
        ...
        (N'/panel/cng/repairs/update', 2, 5),
        (N'/panel/cng/repairs/delete', 2, 6),
        (N'/panel/cng/repairs/confirm', 2, 1000),
        (N'/panel/cng/repairs/sendaspartentrynotify', 2, 1000)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES (N'ShowAllMenus'), (N'Cng.Repairs.Confirm')) r(RoleName);

    -- Children list/view for all roles that can open page
    INSERT INTO #Map SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/cng/repairspart/listbyparentid', 1, 1),
        (N'/panel/cng/repairspart/getlistbyparentid', 2, 2),
        (N'/panel/cng/repairsmanhours/listbyparentid', 1, 1),
        (N'/panel/cng/repairsmanhours/getlistbyparentid', 2, 2),
        (N'/panel/cng/repairsattachment/listbyparentid', 1, 1),
        (N'/panel/cng/repairsattachment/getlistbyparentid', 2, 2)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES
        (N'ShowAllMenus'), (N'مشاهده کلی HTS'),
        (N'Cng.Repairs.Manage'), (N'Cng.Repairs.Confirm'), (N'Cng.Repairs.View')
    ...
    -- Children editable for Manage + Confirm (+ ShowAll)
    INSERT INTO #Map SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/cng/repairspart/new', 1, 4),
        (N'/panel/cng/repairspart/edit', 1, 5),
        (N'/panel/cng/repairspart/save', 2, 3),
        (N'/panel/cng/repairspart/add', 2, 4),
        (N'/panel/cng/repairspart/update', 2, 5),
        (N'/panel/cng/repairsmanhours/new', 1, 4),
        (N'/panel/cng/repairsmanhours/edit', 1, 5),
        (N'/panel/cng/repairsmanhours/save', 2, 3),
        (N'/panel/cng/repairsmanhours/add', 2, 4),
        (N'/panel/cng/repairsmanhours/update', 2, 5),
        (N'/panel/cng/repairsmanhours/delete', 2, 6),
        (N'/panel/cng/repairsattachment/new', 1, 4),
        (N'/panel/cng/repairsattachment/edit', 1, 5),
        (N'/panel/cng/repairsattachment/save', 2, 3),
        (N'/panel/cng/repairsattachment/add', 2, 4),
        (N'/panel/cng/repairsattachment/update', 2, 5),
        (N'/panel/cng/repairsattachment/delete', 2, 6)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES (N'ShowAllMenus'), (N'Cng.Repairs.Manage'), (N'Cng.Repairs.Confirm')) r(RoleName);

    DELETE ra FROM system.RoleAccess ra
    INNER JOIN system.Role r ON r.Id = ra.RoleId
    WHERE (
            ra.Path LIKE N'/panel/cng/repairs/%'
         OR ra.Path LIKE N'/panel/cng/repairspart/%'
         OR ra.Path LIKE N'/panel/cng/repairsmanhours/%'
         OR ra.Path LIKE N'/panel/cng/repairsattachment/%'
      )
      AND r.Name IN (
            N'Cng.Repairs.Manage', N'Cng.Repairs.Confirm', N'Cng.Repairs.View',
            N'ShowAllMenus', N'مشاهده کلی HTS'
    ...
    SELECT DISTINCT m.Path, m.AType, m.IType,
        CASE
            WHEN m.Path LIKE N'/panel/cng/repairspart/%' THEN N'Entities.App.Cng.RepairsPart'
            WHEN m.Path LIKE N'/panel/cng/repairsmanhours/%' THEN N'Entities.App.Cng.RepairsManHours'
            WHEN m.Path LIKE N'/panel/cng/repairsattachment/%' THEN N'Entities.App.Cng.RepairsAttachment'
            ELSE N'Entities.App.Cng.Repairs'
        END,
        NULL, NULL, NULL, r.Id,
    ...
    PRINT N'=== [4] Assign HTS page 383 users (if TMS linked) ===';
    ...
    BEGIN
        ...
        SELECT DISTINCT COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')), N'Cng.Repairs.Manage'
        ...
        WHERE pug.UserGroup_FK = 231 AND pa.Page_FK = 383
          AND COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) IS NOT NULL;

        -- 232 Confirm
        INSERT INTO #UserRoleMap (Username, RoleName)
        SELECT DISTINCT COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')), N'Cng.Repairs.Confirm'
        FROM [TMS].[TotalSystem].[dbo].[Gnr_PageAction_UserGroup] pug
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_PageAction] pa ON pa.PageAction_ID = pug.PageAction_FK
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_UserGroupMember] m ON m.UserGroup_FK = pug.UserGroup_FK
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_User] u ON u.User_ID = m.User_FK
        WHERE pug.UserGroup_FK = 232 AND pa.Page_FK = 383
          AND COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) IS NOT NULL
          AND NOT EXISTS (SELECT 1 FROM #UserRoleMap x WHERE x.Username = COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) AND x.RoleName = N'Cng.Repairs.Confirm');
        ...
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_UserGroupMember] m ON m.UserGroup_FK = pug.UserGroup_FK
        INNER JOIN [TMS].[TotalSystem].[dbo].[Gnr_User] u ON u.User_ID = m.User_FK
        CROSS JOIN (VALUES (N'Cng.Repairs.View'), (N'مشاهده کلی HTS')) v(RoleName)
        WHERE pug.UserGroup_FK = 350 AND pa.Page_FK = 383
          AND COALESCE(NULLIF(LTRIM(RTRIM(u.ActiveDirectoryUsername)), N''), NULLIF(LTRIM(RTRIM(u.Username)), N'')) IS NOT NULL
    ...
    COMMIT TRANSACTION;
    PRINT N'=== DONE Seed_CngRepairs_RolesAndAccess ===';
END TRY
BEGIN CATCH
...
SELECT Id, Name, Title FROM system.Role WHERE Name LIKE N'Cng.Repairs%' ORDER BY Id;
SELECT r.Name, COUNT(*) AS Acc
FROM system.RoleAccess ra INNER JOIN system.Role r ON r.Id = ra.RoleId
WHERE ra.Path LIKE N'/panel/cng/repairs%'
