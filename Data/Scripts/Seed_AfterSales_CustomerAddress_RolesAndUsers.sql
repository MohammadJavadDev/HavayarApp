/*
  Seed_AfterSales_CustomerAddress_RolesAndUsers.sql
  سایت های مشتریان مطابق HTS صفحه 234 Sale_Customer_Address:
    گروه 48 + 350 Read → View
    Edit/Full فقط sharifi.m / kalhor.m / mohammadi.sam (و اعطای مستقیم HTS perm 2/5)
    Sale.AfterSales (500000 / گروه 69) از این صفحه حذف می‌شود.
    اکسل برای مشاهده‌کنندگان هم فعال است.
...
BEGIN TRY
    ...
    DECLARE @SeedUser NVARCHAR(150) = N'seed-aftersales-customeraddress-roles';
    DECLARE @EntityPascal NVARCHAR(200) = N'Entities.App.SLS.CustomerAddress';

    PRINT N'=== [1] Roles 500028 / 500029 ===';
    IF OBJECT_ID('tempdb..#CaRoles') IS NOT NULL DROP TABLE #CaRoles;
    CREATE TABLE #CaRoles (WantId BIGINT NOT NULL, Name NVARCHAR(200) NOT NULL, Title NVARCHAR(200) NOT NULL);
    INSERT INTO #CaRoles (WantId, Name, Title) VALUES
    ...
    DECLARE rc CURSOR LOCAL FAST_FORWARD FOR SELECT WantId, Name, Title FROM #CaRoles;
    ...
    PRINT N'=== [2] RoleAccess (strip AfterSales 500000 from this page) ===';
    DELETE ra
    FROM system.RoleAccess ra
    WHERE ra.Path LIKE N'/panel/sls/customeraddress/%'
       OR (ra.ActionAccessType = 3 AND ra.EntityName = @EntityPascal);

    IF OBJECT_ID('tempdb..#CaActionMap') IS NOT NULL DROP TABLE #CaActionMap;
    CREATE TABLE #CaActionMap
    (
    ...
    INSERT INTO #CaActionMap (RoleName, Path, ActionAccessType, ActionAccessItemType)
    SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/sls/customeraddress/list', 1, 1),
        (N'/panel/sls/customeraddress/fetchdata', 2, 2),
        (N'/panel/sls/customeraddress/exporttoexcel', 2, 0),
        (N'/panel/sls/customeraddress/edit', 1, 5)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES
        (N'ShowAllMenus'),
    ...
    SELECT r.RoleName, a.Path, a.AType, a.IType
    FROM (VALUES
        (N'/panel/sls/customeraddress/save', 2, 3),
        (N'/panel/sls/customeraddress/update', 2, 5)
    ) a(Path, AType, IType)
    CROSS JOIN (VALUES
        (N'ShowAllMenus'),
        (N'Sale.AfterSales.CustomerAddress.Manage')
    ...
    PRINT N'=== [3] Menu AccessRoleIds merge + HTS caption ===';
    ...
    BEGIN
        ...
        SET Content = REPLACE(REPLACE(Content, N'سایت‌های مشتریان', N'سایت های مشتریان'), N'سایت مشتری', N'سایت های مشتریان'),
            ModifiedById = 1, ModifiedByName = @SeedUser,
            ModifiedDateMiladiDateTime = @Now, ModifiedDateShamsiDateTime = @NowShamsi
        WHERE Id = @MenuId AND Content LIKE N'%سایت%';
    ...
    PRINT N'=== [4] Assign HTS page 234 users ===';
    ...
    WHERE pa.Page_FK = 234
      AND pug.UserGroup_FK IN (48, 350);
    ...
    FROM (
        SELECT COALESCE(g.AdName, g.HtsUsername) AS Username,
               N'Sale.AfterSales.CustomerAddress.Manage' AS RoleName, g.Source
        FROM #HtsGrant g
        WHERE g.Permission_ID IN (2, 5) AND COALESCE(g.AdName, g.HtsUsername) IS NOT NULL

        UNION ALL
        SELECT COALESCE(g.AdName, g.HtsUsername),
               N'Sale.AfterSales.CustomerAddress.View', g.Source
        FROM #HtsGrant g
    ...
    INSERT INTO #UserRoleMap (Username, RoleName, Source)
    SELECT v.Username, N'Sale.AfterSales.CustomerAddress.Manage', N'explicit-hts-editor'
    FROM (VALUES (N'sharifi.m'), (N'kalhor.m'), (N'mohammadi.sam'), (N'Mohammadi.sam')) v(Username)
    WHERE NOT EXISTS (
        SELECT 1 FROM #UserRoleMap x
        WHERE LOWER(x.Username) = LOWER(v.Username) AND x.RoleName = N'Sale.AfterSales.CustomerAddress.Manage'
    );
    ...
    DECLARE @ViewRoleId bigint = (SELECT Id FROM system.Role WHERE Name = N'Sale.AfterSales.CustomerAddress.View');
    DECLARE @ManageRoleId bigint = (SELECT Id FROM system.Role WHERE Name = N'Sale.AfterSales.CustomerAddress.Manage');
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT @StripRoles = N'[' + STUFF((
                ...
                WHERE j.value <> N'Sale.AfterSales.CustomerAddress.View'
                FOR XML PATH(''), TYPE).value('.', 'nvarchar(max)'), 1, 1, N'') + N']';
    ...
    COMMIT TRANSACTION;
    PRINT N'=== DONE Seed_AfterSales_CustomerAddress_RolesAndUsers ===';
END TRY
BEGIN CATCH
...
SELECT r.Id, r.Name, r.Title,
       (SELECT COUNT(*) FROM system.RoleAccess ra
        WHERE ra.RoleId = r.Id AND ra.Path LIKE N'/panel/sls/customeraddress/%') AS CaAccess,
